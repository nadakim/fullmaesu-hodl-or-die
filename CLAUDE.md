...
- 현재 로직 영역(`docs/demo`의 `<script>` 안):
  - 상태: `availableCash`, `realizedPnL`, `marketPrice`, `marketState`, `openPositions`, `handCards`, `candleData`
  - 데이터: `CARD_DATABASE`, `MARKET_EVENTS`
  - 매매: `buyPosition`, `sellPosition`, `sellAllPositions`
  - 가치 계산: `calculatePositionValue`, `generateNextCandle`
  - 이벤트: `triggerMarketEvent`
  - 일부 로직 함수가 아직 `alert`/DOM/`renderUI()`를 직접 호출한다. 해당 함수를 수정할 때 UI 호출을 바깥으로 빼는 방향으로 정리한다(요청 범위 안에서만).
...
3. 최소 스모크 시나리오:
   ...
   - `#handBox .card` 하나 클릭 → 현금이 카드 비용만큼 줄고 `#positionsBox`에 포지션이 생기는지
   - `◆ 전량 매도` 클릭 → 포지션이 0개(`NO POSITION`)가 되고 현금/확정손익이 갱신되는지
   - 페이지 에러(`pageerror`)가 없는지
4. 변경한 기능 자체도 직접 조작해 확인하고, 필요하면 스크린샷으로 레이아웃을 본다.

참고: 차트가 800ms마다 갱신되며 손패(`#handBox`)를 다시 그리므로, 요소 핸들을 오래 들고 있지 말고 locator로 매번 새로 찾는다.
